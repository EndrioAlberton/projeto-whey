...
export async function getOpcoesFiltros() {
  const [marcas, sabores, tamanhos] = await Promise.all([
    fetch(`${API_URL}/api/marcas/`,    { next: { revalidate: 3600 } }).then(r => r.json()),
    fetch(`${API_URL}/api/sabores/`,   { next: { revalidate: 3600 } }).then(r => r.json()),
    fetch(`${API_URL}/api/tamanhos/`,  { next: { revalidate: 3600 } }).then(r => r.json()),
  ])
  return {
